 ...
 *   POST   /start            -> { username, maxVideos, uniquify, translate } -> { id }
...
function createAutoCloneRouter(express, context = {}) {
  ...
  router.post("/start", route(async (req, res) => {
    const { username, maxVideos, uniquify, translate, minViews, destinationRoot, uniquifyIntensity, uniquifyOptions } = req.body || {};
    const result = await controller.start({ username, maxVideos, uniquify, translate, minViews, destinationRoot, uniquifyIntensity, uniquifyOptions });
