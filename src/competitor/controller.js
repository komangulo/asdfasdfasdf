...
class CompetitorController extends EventEmitter {
  constructor() {
    ...
    this.running = false;
    this.settings = { geminiApiKey: "", model: aiAnalyst.DEFAULT_MODEL, brand: "", language: "es" };
  ...
  async listModels() {
    if (!this.settings.geminiApiKey) throw new Error("Falta la API key de Gemini.");
    const models = await aiAnalyst.listModels(this.settings.geminiApiKey);
    return models;
  }

  async saveSettings(patch = {}) {
    if (typeof patch.geminiApiKey === "string") {
      if (patch.geminiApiKey.trim()) this.settings.geminiApiKey = patch.geminiApiKey.trim();
      else if (patch.geminiApiKey === "") this.settings.geminiApiKey = "";
    }
  ...
  async analyze({ target, depth, brand, language } = {}) {
    ...
    if (!this.settings.geminiApiKey) throw new Error("Falta la API key de Gemini. Añádela en la sección Competencia.");
    this.running = true;
    let resolvedHandle = "";
    ...
    try {
      ...
      this._report("analyzing", `Analizando ${report.videos.length} vídeos con Gemini...`, { handle: resolvedHandle, percent: 65 });
